...
## 1.7.2-tte - 2024-04-30

### Changed

* Added to HUD info at top-center: a) targeted altitude with 2 decimals; b) route name and next index and waypoint name (if exists) when flying a route (published RouteInfo)
* WASD class: smaller target margin on movement; in change() always monitor (new) height
* Point class: added parsesPos to reduce costly ParsePosition calls in several other classes
* Waypoint: with autoPitch enabled, in Pitch() decreased 75 to 60 degrees
* Added "b.cmd" to compile only release version and copy it to clipboard if succesfull
* Project: only builds Unlimited version now as variants are no longer needed
* Coding style: using "si" for singleton instead of "s" in some classes to avoid intellisense warnings
* Added short "tte" to the version on screen to note my version
